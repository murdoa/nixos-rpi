...
# decompresses gzip Images into a high scratch region before relocating the
# resulting ARM64 Image to its final address.
...
addEntry() {
    ...
    if [ "$raw" = 1 ]; then
        ...
        copyCompressedKernelToKernelsDir "$path/kernel"; kernel=$result
    fi
...
# Keep an uncompressed entry for the current generation while compressed boot
# is being proven. It is selectable after interrupting U-Boot autoboot.
addEntry $default default-raw 1 >> $tmpFile
