              ...
              nixpkgs.overlays = [
                (final: prev: {
                  ...
                  timebase_hmi = timebaseHmi.packages.${targetSystem}.default.overrideAttrs (oldAttrs: {
                    flutterBuildFlags = (oldAttrs.flutterBuildFlags or [ ]) ++ [
                      "--dart-define=GALACTICA_NODE_ID=${nodeId}"
                    ];
      ...
      nixosConfigurations = {
        ...
        quark-display = mkPiConfig {
          ...
          modules = [
            "${nixpkgs}/nixos/modules/profiles/minimal.nix"
            ./configuration.nix
            { networking.hostName = "quark-display"; }
          ];
        };

        sisko-display = mkPiConfig {
          buildSystem = "aarch64-linux";
          targetSystem = "aarch64-linux";
          nodeId = "sisko";
          specialArgs = {
          ...
          modules = [
            "${nixpkgs}/nixos/modules/profiles/minimal.nix"
            ./configuration.nix
            { networking.hostName = "sisko-display"; }
          ];
        };
      };
    };
