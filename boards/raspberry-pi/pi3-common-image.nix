...
let
  ...
  installBootLoader = pkgs.writeShellScript "install-raspberry-pi-boot-partition" ''
    ...
    mkdir -p "$tmp/EFI/BOOT" "$tmp/EFI/systemd" "$tmp/EFI/nixos" "$tmp/loader/entries"
    ...
    cp --no-preserve=mode,ownership ${lib.escapeShellArg efiSource} "$tmp/EFI/systemd/systemd-boot${efiArch}.efi"
